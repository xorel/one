...
void VirtualNetworkXML::init_attributes()
{
    ...
    //-------------------- AR_POOL used leases ------------------------------
    //vector<string> ar_ids;
    vector<string> ar_size;
    vector<string> ar_used_leases;

    //xpaths(ar_ids, "/VNET/AR_POOL/AR/AR_ID");
    ...
    for (size_t i = 0; /*i < ar_ids.size() &&*/ i < ar_size.size() ; i++)
    {
        //id          = atoi(ar_ids[i].c_str());
        size        = atoi(ar_size[i].c_str());

        //free_leases[id] = size - used_leases;
        free_leases += size;
    }
...
ostream& operator<<(ostream& o, const VirtualNetworkXML& p)
{
    ...
    o << "ID          : " << p.oid          << endl;
    o << "FREE_LEASES : " << p.free_leases  << endl;
