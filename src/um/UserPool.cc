                                     ...
                                     int&          umask)
{
    ...
    // Search previous groups where user was admin
    ...
    if ( !driver_managed_groups || new_gid == -1 || ( new_group_ids == group_ids && group_admin_ids == new_group_admin_ids ) )
    {
        user->unlock();
        return true;
    }
    ...
    // -------------------------------------------------------------------------
    // For groups in which user was before and still remains also check if the
    // admin status did not changed
    // -------------------------------------------------------------------------
